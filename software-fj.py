...
# Proyecto colaborativo - Fase 4 - Programación (213023) - UNAD
#-----------------------------------------------------------------------#
...
class Reserva(EntidadBase):
    ...
    def cancelar(self):
        # COMMIT 9: Redondear la penalizacion para evitar decimales largos
        if self._estado == "Confirmada":
            self._costo_total = round(self._costo_total * 0.2, 2)
...
for caso in range(1, 11):
    ...
    try:
        if caso == 1:
            ...
            # COMMIT 9: caso invalido, se espera que falle por email sin formato
            c = Cliente("C02", "Carlos Perez", "correo")
        elif caso == 3:
            s1 = ReservaSala("S01", "Sala VIP", 50000, 12)
            s2 = AlquilerEquipo("S02", "Proyector", 30000, 15000)
            servicios.append(s1)
            servicios.append(s2)
            print("  OK -> Servicios creados con exito.")
        elif caso == 4:
            s3 = AsesoriaEspecializada("S03", "Asesoria Ciber", 120000, "Sistemas")
            servicios.append(s3)
            print("  OK -> Asesoria especializada configurada.")
        elif caso == 5:
            ...
            # COMMIT 8: caso invalido, se espera ReservaInvalidaError por horas negativas
            r_err = Reserva("R02", clientes[0], servicios[2], -2)
            r_err.procesar_y_confirmar()
        elif caso == 7:
            r = reservas[0]
            r.cancelar()
            print(f"  OK -> Reserva cancelada. Penalizacion: ${r.costo_total}")
        elif caso == 8:
            # COMMIT 8: caso invalido, se espera error al reconfirmar una reserva cancelada
            r = reservas[0]
            ...
            # COMMIT 9: caso invalido, se espera que falle por nombre vacio
        ...
        registrar_evento(f"Error inesperado caso {caso}: {e}", es_error=True)
    else:
...
try:
    ...
    print("Aun no se ha generado el archivo de logs.")
